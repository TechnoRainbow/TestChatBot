...
Система предоставляет консультации по инвестиционным продуктам, используя векторный поиск в базе знаний и генерацию ответов через LLM. Реализован полноценный RAG-пайплайн с поддержкой FAISS для эмбеддингов.
...
### Требования
- Python 3.8+
- 2GB RAM (для загрузки embedding модели)
...
- **Время инициализации**: ~10-15 секунд (загрузка embedding модели)
- **Время ответа**: 2-5 секунд (зависит от LLM API)
- **Точность поиска**: Косинусное сходство с порогом 0.3
- **Размер индекса**: ~16 документов, 384-мерные векторы

## 🔒 Обработка ошибок

Система включает многоуровневую обработку ошибок:

1. **Валидация входящих данных** (Pydantic)
2. **Retry логика** для LLM API (3 попытки)
3. **Fallback сценарии** при недоступности LLM
4. **Graceful degradation** с информацией из базы знаний
5. **Логирование** всех ошибок для диагностики
...
- Ограничение длины запросов (1000 символов)
- Логирование запросов для аудита
- Обфускация API токенов в логах
